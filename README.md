...
```bash
git clone https://github.com/WeaveSolicitor/mkdev-123.git
cd mkdev-123
python setup.py
```
...
Pre-built binaries for macOS (Intel + Apple Silicon), Linux (amd64 + arm64), and Windows (amd64) are published on the [Releases page](). Each release includes `checksums.txt` plus a cosign keyless signature (`checksums.txt.sig` + `.pem`) — see [SECURITY.md#verifying-releases](./SECURITY.md) for the verify command.
...
<!-- Last updated: 2026-06-06 14:47:53 -->
